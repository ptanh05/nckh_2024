...
// Function to savse transaction activity in localStorage
const saveActivity = (
  type: "lock" | "unlock",
...
export default function LockAssets() {
  ...
  const [nftAssets, setNftAssets] = useState<NftAssetInput[]>([
    { policyId: "", assetName: "", quantity: "1" },
  ]);
    ...
    const fetchTransactionHistory = async () => {
      ...
      try {
        ...
        if (!txResponse.ok) {
          ...
          throw new Error(
            `Error fetching transaction history: ${txResponse.status}`
          );
        }
  ...
  const updateNftAsset = (
    index: number,
    field: keyof NftAssetInput,
    value: string
  ) => {
    ...
    for (const asset of nftAssets) {
      if (asset.policyId || asset.assetName || asset.quantity !== "1") {
        if (!asset.policyId || !asset.assetName) {
          setError(
            "All NFT fields (Policy ID and Asset Name) must be filled in."
          );
          return false;
        }
    ...
    <main>
      <div className="container mx-auto px-4 py-8">
        ...
        <p className="text-gray-400 mb-8">
          Securely lock your digital assets with time-based conditions and
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          updateNftAsset(index, "policyId", e.target.value)
                        }
                        disabled={isLoading}
                      />
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          updateNftAsset(index, "assetName", e.target.value)
                        }
                        disabled={isLoading}
                      />
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          updateNftAsset(index, "quantity", e.target.value)
                        }
                        disabled={isLoading}
                      />
                    </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white/5 rounded-lg p-4">
                ...
                <p className="text-gray-300">
                  {lovelace ? parseFloat(lovelace).toLocaleString() : "0"} ₳
                </p>
              </div>
              ...
              <div className="bg-white/5 rounded-lg p-4">
                ...
                {nftAssets.some(
                  (asset) =>
                    asset.policyId || asset.assetName || asset.quantity !== "1"
                ) ? (
                  nftAssets.map((asset, index) => (
                        ...
                        <p className="text-gray-300">
                          ...
                          {asset.policyId && asset.assetName && (
                            <>
                              ...
                              <span className="break-all">
                                {asset.policyId}
                                {asset.assetName.match(/^[0-9a-fA-F]*$/)
                                  ? asset.assetName
                                  : Buffer.from(asset.assetName).toString(
                                      "hex"
                                    )}
                              </span>
                            </>
              ...
              {txHash && (
                <div
                  className={`bg-white/5 rounded-lg p-4 ${
                    transactionSuccess ? "border border-green-500" : ""
                  }`}
                >
                  <h3 className="text-lg font-medium mb-2">
                    Transaction Result
                  ...
                  <p className="text-gray-300">
                    ...
                    <br />
                    <code className="block bg-black/30 p-2 rounded mt-1 overflow-auto break-all">
                      {txHash}
                    </code>
                  </p>
                ...
                <tbody>
                  {transactionHistory.map((tx) => (
                    <tr key={tx.id} className="border-b border-gray-800/50">
                      <td className="py-4">{`${parseFloat(
                        tx.amount
                      ).toLocaleString()} ADA`}</td>
